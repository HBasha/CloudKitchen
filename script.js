...
function renderCategories(categories) {
  ...
  tabs.innerHTML = `<button class='w-full px-4 py-3 text-lg bg-blue-600 text-white rounded-2xl' onclick="changeCategory('All')">All</button>`;

  categories.forEach(cat => {
    tabs.innerHTML += `<button class='w-full px-4 py-3 text-lg bg-gray-200 rounded-2xl' onclick="changeCategory('${cat.name}')">${cat.name}</button>`;
  });
}
...
function renderMenu() {
  ...
  fullData.categories.forEach(cat => {
    ...
    cat.items.filter(i => i.name.toLowerCase().includes(query)).forEach(item => {
      ...
      card.className = `bg-white p-6 rounded-2xl shadow-xl ${!available ? 'opacity-40' : ''}`;

      card.innerHTML = `
        <img src='${item.image}' class='w-full h-52 object-cover rounded-xl'>
